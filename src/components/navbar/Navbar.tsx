...
function Navbar({
  ...
  return (
    <motion.div
      ...
      className={`fixed z-[9999] rounded-[20rem] bg-transparent bg-opacity-50  ${
        !state1 && "backdrop-blur-sm"
    ...
    >
      ...
      <motion.nav
        className="flex flex-row gap-[15rem] md:w-max w-0 overflow-hidden"
        initial={{ scale: 0 }}
        animate={controls}
        transition={{ duration: 1, type: "spring" }}
      >
        <div className="flex flex-row grow  gap-[2rem]">
          <Link name="Services" link="/services" />
          <Link name="Our Work" link="/work" />
          <Link name="About us" link="/about" />
          <Link name="Insights" link="/insights/all" />
        </div>
